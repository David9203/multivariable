...
row.names(datos) <- datos[,c(1)]
 ...
 correlationanalisis<-function(datanum){
   ...
   Data <- subset( datos, select = -foreign ) ##saco la var long por que no aporta nada
      Data <- subset( Data, select = -num ) ##saco la var long por que no aporta nada
...
2. #Homoscedasticidad
x<-scores[,c(1:2)]
bartlett.test(list(scores[,1],scores[,2]))
...
res.pca <- prcomp(valuesbox, scale = TRUE)
res.pca

#solicito las tablas
get_pca_var(res.pca)
get_eig(res.pca)#tabla de eigenvalores varianza acumulada
valpca=res.pca$x[,1:2]
correlationanalisis(valpca)
